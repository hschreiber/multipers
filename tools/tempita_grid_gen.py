...
OUTPUT_ROOT = Path(
    os.environ.get(
        "MULTIPERS_TEMPITA_GRID_OUTPUT_ROOT", str(REPO_ROOT / "build" / "generated")
    )
).resolve()
...
def _render_instantiations_include_filt(type_names: list[str]) -> str:
    lines = [
        "// Auto-generated by tools/tempita_grid_gen.py. Do not edit.",
        "",
    ]
    lines.extend(
        f"template<> void {type_name}::set_num_generators(std::size_t) = delete;"
        for type_name in type_names
        if type_name[type_name.rfind(",") + 2 : -1] == "false"
    )
...
def _render_instantiations_include_slicer(type_names: list[str]) -> str:
    lines = [
        "// Auto-generated by tools/tempita_grid_gen.py. Do not edit.",
        "",
    ]
...
def _render_instantiations_include_simplextree(type_names: list[str]) -> str:
    lines = [
        "// Auto-generated by tools/tempita_grid_gen.py. Do not edit.",
        "",
    ]
...
def _render_pairwise_template_header(
    template_name: str, target_types: list[str], source_types: list[str]
) -> str:
    lines = [
        "#pragma once",
        "",
    ...
    for target_type in target_types:
        for source_type in source_types:
            lines.append(
                f"extern template struct multipers::core::{template_name}<{target_type}, {source_type}>;"
            )
    lines.append("")
    return "\n".join(lines)


def _render_pairwise_instantiations_include(
    template_name: str, target_types: list[str], source_types: list[str]
) -> str:
    lines = [
        "// Auto-generated by tools/tempita_grid_gen.py. Do not edit.",
    ...
    for target_type in target_types:
        for source_type in source_types:
            lines.append(
                f"template struct multipers::core::{template_name}<{target_type}, {source_type}>;"
            )
    lines.append("")
    return "\n".join(lines)


def _render_slicer_nanobind_registry(
    slicers: list[dict[str, Any]], simplextrees: list[dict[str, Any]]
) -> str:
    lines = [
        "// Auto-generated by tools/tempita_grid_gen.py. Do not edit.",
        "",
    ...
    slicer_by_python = {slicer["PYTHON_TYPE"]: slicer for slicer in slicers}

    for index, slicer in enumerate(slicers):
        coarsened = slicer_by_python[slicer["COARSENNED_PY_CLASS_NAME"]]
        enable_module_approximation = (
            slicer["IS_VINE"]
            and slicer["IS_FLOAT"]
            and slicer["SHORT_VALUE_TYPE"] == "f64"
        )
        enable_bitmap_builder = (
            not slicer["IS_KCRITICAL"]
            and not slicer["IS_VINE"]
            and slicer["SHORT_FILTRATION_TYPE"] == "Contiguous"
            and slicer["PERS_BACKEND_TYPE"] == "Matrix"
            and slicer["COLUMN_TYPE"] == "UNORDERED_SET"
        )
        dtype_name = slicer["PY_VALUE_TYPE"].split(".")[-1]
        lines.extend(
            [
                ...
                f"  static constexpr bool has_grid_methods = {_bool_cpp(slicer['COLUMN_TYPE'] is not None)};",
                f"  static constexpr bool enable_module_approximation = {_bool_cpp(enable_module_approximation)};",
                f"  static constexpr bool enable_bitmap_builder = {_bool_cpp(enable_bitmap_builder)};",
                f'  static constexpr std::string_view python_name = "{slicer["PYTHON_TYPE"]}";',
                f'  static constexpr std::string_view dtype_name = "{dtype_name}";',
                f'  static constexpr std::string_view column_type = "{slicer["COLUMN_TYPE"]}";',
                f'  static constexpr std::string_view filtration_container = "{slicer["SHORT_FILTRATION_TYPE"]}";',
                f'  static constexpr std::string_view backend_type = "{slicer["PERS_BACKEND_TYPE"]}";',
                f'  static constexpr std::string_view filtration_type = "{slicer["FILTRATION_TYPE"]}";',
                f'  static constexpr std::string_view short_value_type = "{slicer["SHORT_VALUE_TYPE"]}";',
                "};",
                "",
            ]
        )
    ...
    simplextree_names = ", ".join(
        f"SimplexTreeDesc_{index}" for index in range(len(simplextrees))
    )
...
def _render_mma_nanobind_registry(value_types: list[tuple[str, str, str]]) -> str:
    lines = [
        "// Auto-generated by tools/tempita_grid_gen.py. Do not edit.",
        "",
    ]

    for index, (ctype, pytype, short) in enumerate(value_types):
        dtype_name = pytype.split(".")[-1]
        lines.extend(
            [
...
slicer_conversion_instantiation_count = len(slicer_instantiation_types) * len(
    slicer_instantiation_types
)
simplextree_conversion_instantiation_count = len(simplextree_instantiation_types) * len(
    simplextree_instantiation_types
)
...
_write_text_if_changed(
    ...
    _render_instantiations_include_filt(filtration_instantiation_types),
)
_write_text_if_changed(
...
_write_text_if_changed(
    OUTPUT_ROOT / "multipers/_slicer_nanobind_registry.inc",
    _render_slicer_nanobind_registry(slicers, st_list),
)
_write_text_if_changed(
    OUTPUT_ROOT / "multipers/_mma_nanobind_registry.inc",
    _render_mma_nanobind_registry(value_types),
)
